window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var DEFAULT_PREFS = { enabled: true, interval: 60, providerCheckSec: 5 };
    ...
    function useCurrentProvider(sessionId, hasSessionsHook, ctx, checkSec) {
      ...
      React.useEffect(() => {
        ...
        check();
        const disposer = ctx.interval(check, Math.max(2, checkSec) * 1e3);
        return () => {
          cancelled = true;
    ...
    function BalanceBadge({ scope, ctx, useSessions }) {
      ...
      const list = useSessions ? useSessions((s) => s) : void 0;
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              { onClick: () => {
                refresh();
                recheck();
              }, style: { background: "transparent", border: "none", color: "#6ab0f3", cursor: "pointer", fontSize: 12, padding: 0 } },
